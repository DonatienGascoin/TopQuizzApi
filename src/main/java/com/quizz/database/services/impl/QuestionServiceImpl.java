...
public class QuestionServiceImpl implements QuestionService {
		...
		if (bean != null) {
			...
			if (CollectionUtils.isNotEmpty(bean.getQuizzs())) {
				...
				for (QuizzBean quizzB : bean.getQuizzs()) {
					 ...
					 * Collection<Question> questions2 = new
					 * ArrayList<Question>(); for (QuestionBean question2:
					 * quizzB.getQuestions()) {
					 * questions2.add(getQuestionByQuestionBean(question2)); }
					 * qu.setQuestions(questions2); } quizzs.add(qu);
					 **/
				}
				q.setQuizzs(quizzs);
			}
	...
	public ReturnObject findById(Integer id) {
		...
		try {
			obj.setObject(questionRepository.findOne(id));
		} catch (Exception e) {
	...
	public ReturnObject deleteQuestion(int id) {
		...
		try {
			...
			obj.setCode(ReturnCode.ERROR_050);
			log.error("Impossible to delete question [id: " + id + "]");
		}
		return obj;
	}
}
