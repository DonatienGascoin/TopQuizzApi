...
public class StatisticServiceImpl implements StatisticService {
	...
	public ReturnObject getTenLastScoreForQuizz(String pseudo, Integer quizzId) {
		...
		try {
			...
			Collection<StatisticBean> statisticBeans = statisticRepository
					.findTop10ByPseudoAndAndQuizzIdOrderByDateDesc(pseudo, quizzId);
			if (CollectionUtils.isNotEmpty(statisticBeans)) {
				for (StatisticBean statisticBean : statisticBeans) {
					statistics.add(getStatisticByStatisticBean(statisticBean));
				}
			...
			log.error("Impossible to get Statistic [pseudo: " + pseudo + ",quizzId: " + quizzId + "], "
	...
	public ReturnObject addScoreForQuizz(String pseudo, Integer quizzId, String quizzName, Integer nbRightAnswers,
		...
		try {
			...
			if (sbRet != null) {
				object.setObject(getStatisticByStatisticBean(sbRet));
				object.setCode(ReturnCode.ERROR_000);
			...
			log.error("Impossible to add Statistic [pseudo: " + pseudo + ",quizzId: " + quizzId + "], "
	...
	@Override
	public ReturnObject getStatisticByPseudoAndQuizzId(String targetPseudo, Integer quizzId) {
		log.info("Get statistic for  [pseudo: " + targetPseudo + "quizz Id: " + quizzId + "]");
		ReturnObject object = new ReturnObject();

		try {
			StatisticBean statisticBeans = statisticRepository
					.findTop1ByPseudoAndAndQuizzIdOrderByDateDesc(targetPseudo, quizzId);
			if (statisticBeans != null) {
				object.setObject(getStatisticByStatisticBean(statisticBeans));
				object.setCode(ReturnCode.ERROR_000);
			} else {
				object.setCode(ReturnCode.ERROR_050);
			}
			...
			object.setCode(ReturnCode.ERROR_200);
			log.error("Impossible to get Statistic [pseudo: " + targetPseudo + ",quizzId: " + quizzId + "], "
					+ ReturnCode.ERROR_050);
		} catch (Exception e) {
			object.setCode(ReturnCode.ERROR_050);
			log.error("Impossible to get Statistic [pseudo: " + targetPseudo + ",quizzId: " + quizzId + "], "
	...
	private Statistic getStatisticByStatisticBean(StatisticBean sb) {
